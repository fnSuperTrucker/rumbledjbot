...
function watchChat() {
  ...
  if (!target) {
    console.log('Chat container (#chat-history-list) not found yet, retrying in 1s...');
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    mutations.forEach((mutation) => {
      console.log('Mutation detected, processing', mutation.addedNodes.length, 'new nodes');
...
function processInitialChat(target) {
  const spans = target.querySelectorAll('span.ng-star-inserted:not([data-processed]), span.chat-message:not([data-processed])');
